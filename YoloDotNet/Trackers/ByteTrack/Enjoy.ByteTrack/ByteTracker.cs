...
public sealed class ByteTracker(int frameRate = 30, int trackBuffer = 30, float trackThresh = 0.5f, float highThresh = 0.6f, float matchThresh = 0.8f)
{
	...
	public IList<Track> Predict()
	{
		_frameId++;

		// pool: tracked + lost
		var trackPool = _trackedTracks.Concat(_lostTracks).ToArray();

		// 1. Predição KF
		foreach (var track in trackPool)
			track.Predict();

		List<Track> currentLostTracks = new();

		// 2. Aging: converter tracked -> lost se tempo exceder
		foreach (var track in _trackedTracks)
		{
			// Track ficou sem update neste frame
			if (track.State == TrackState.Tracked)
			{
				track.MarkAsLost();
				currentLostTracks.Add(track);
		...
		// 3. Verificar tracks perdidos para remoção
		List<Track> removed = new();
		foreach (var lost in _lostTracks)
		{
			if (_frameId - lost.FrameId > _maxTimeLost)
			{
				lost.MarkAsRemoved();
				removed.Add(lost);
			}
		...
		_trackedTracks.Clear(); // nenhuma detecção → ninguém está actively tracked

		// Refill _lostTracks without replacing the reference
		var newLost = _lostTracks.Except(removed)
								 .Union(currentLostTracks)
								 .ToList();

		_lostTracks.Clear();
		_lostTracks.AddRange(newLost);

		_removedTracks = _removedTracks.Union(removed).ToList();

		return _lostTracks.Concat(_trackedTracks).Where(t => t.IsActivated).ToList();
	}
