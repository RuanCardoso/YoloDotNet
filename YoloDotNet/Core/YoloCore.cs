	...
	internal class YoloCore(YoloOptions yoloOptions) : IDisposable
	{
		...
		public (IDisposableReadOnlyCollection<OrtValue>, SKSizeI) Run<T>(T image)
		{
			...
			try
			{
				lock (_progressLock)
				{
					SKSizeI originalImageSize = default!;
					if (YoloOptions.ImageResize != ImageResize.None)
					{
						originalImageSize = YoloOptions.ImageResize == ImageResize.Proportional
					...
					{
						SKImage originalImage = default!;
						if (image is SKImage skImage)
						{
							originalImageSize = new SKSizeI(skImage.Width, skImage.Height);
							originalImage = skImage;
						}
						else if (image is SKBitmap skBitmap)
						{
							originalImageSize = new SKSizeI(skBitmap.Width, skBitmap.Height);
							originalImage = SKImage.FromPixels(skBitmap.Info, skBitmap.GetPixels());
						}

						var rect = new SKRect(0, 0, originalImageSize.Width, originalImageSize.Height);
						pinnedBuffer.Canvas.DrawImage(originalImage, rect, rect, YoloOptions.SamplingOptions);
					}
		...
		{
			if (YoloOptions.ImageResize == ImageResize.None)
				return (0f, 0f, 1f, 1f); // No resize → no padding, no scaling. All bbox/keypoint code must ignore these.

			return YoloOptions.ImageResize == ImageResize.Proportional ? CalculateProportionalGain(size) : CalculateStretchedGain(size);
		}
